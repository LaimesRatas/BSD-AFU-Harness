...
| Supersingular (a_p = 0) | supersingular p | M0_IMC_SUPERSINGULAR_AP0 (or umbrella M0_IMC_SUPERSINGULAR_PLUSMINUS) + W0_KATO_R0 | AFU-2 (A0_w → A0_m) | Signed Iwasawa theory (Pollack/Kobayashi); Wan-style results |
| Supersingular (a_p ≠ 0) | supersingular p | M0_IMC_SUPERSINGULAR_APNE0 (signed beyond classical ±) + W0_KATO_R0 | AFU-2 (A0_w → A0_m) | “Multi-signed / signed” frameworks (Sprung-type, etc.) |
| Additive/multiplicative reduction at ℓ \mid N | p ∈ S_AFU | L_BAD_REDUCTION_LOCAL | AFU-1G (local at bad primes) | Local condition conventions and Tamagawa matching |
...
**Note on periods / Manin constant.** Period scaling is tracked by the C* layer
(lambda_trans(period), C0→C1→C2) in the normalization stack, not as a separate AFU plug-in.
...
| Rank 0 (Index-ID, supersingular a_p=0) | supersingular | M0_IMC_SUPERSINGULAR_AP0 (or umbrella M0_IMC_SUPERSINGULAR_PLUSMINUS) | A0_m | Signed (±) theory |
| Rank 0 (Index-ID, supersingular a_p≠0) | supersingular | M0_IMC_SUPERSINGULAR_APNE0 | A0_m | Signed theory beyond classical ± |
