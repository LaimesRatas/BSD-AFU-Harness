...
\begin{remark}[Dependencies and scope]
\label{rem:dependencies}
This note proves the \URC\ (Bulk--Edge) locking implication in the BSD determinant line: the global defect scalar $u(E)\in\Q^\times$
is principal and, after adelic valuation collapse and canonical sign calibration, satisfies $u(E)=1$ (hence $u^{\mathrm{glob}}_p(E)=1$ for all $p$).
The argument depends on the \DLT\ package only through the existence of a $\Q$-linear transport map on determinant lines and its compatibility with base change to $\Q_v$,
so that the local factors $u_v$ are the images of a single global $u(E)$.
The vanishing of non-$p_0$ valuations $v_\ell(u(E))=0$ for $\ell\neq p_0$ depends on the \LAI\ (Gate~L) contract: unramified places contribute canonically via the unramified lattice, and ramified places are absorbed by the Tamagawa normalization in the integral structure.
These inputs are treated modularly here (as admissible Gate packages), rather than re-derived from scratch.
The arithmetic upgrade \AFU\ is strictly separate: \emph{A3-Int} begins exactly when one passes from unit/normalization locking to integral index control and its identification with BSD factors (including any $\Sha$-finiteness/rank-closure requirements).
\end{remark}
...
We fix the canonical \emph{Bloch--Kato fundamental line} over $\Q$, denoted $\Delta_{\mathrm{BK},\Q}(E)$, attached to $E$
(i.e.\ the determinant line of the Selmer/Bloch--Kato complex in the determinant-functor formalism).
For each prime $p$ we set
\[
\Delta_{\mathrm{BK},p}(E)\ :=\ \Delta_{\mathrm{BK},\Q}(E)\otimes_{\Q}\Q_p\ \simeq\ \detline_{\Q_p}\!\bigl(R\Gamma_f(\Q,V_p(E))\bigr),
\]
and equip it with its canonical $\Z_p$-lattice
\[
...
\begin{proof}
Fix $\ell\neq p_0$. If $\ell\notin S_{\mathrm{bad}}$, the local block is unramified and the Gate~L normalization is compatible
with the canonical unramified integral lattice, so the resulting local factor is an $\ell$-adic unit (see \Cref{app:unramified}
and \cite{BlochKato1990,Nekovar2006}). If $\ell\in S_{\mathrm{bad}}$, the claim follows from \Cref{prop:LAI-tamagawa}.
\end{proof}
...
\begin{proof}
This is a bookkeeping consequence of the Gate~L normalization:the canonicity comes from the standard integral structure on the Bloch--Kato fundamental line defined via the N\'eron model at $\ell$ (so the Tamagawa correction is built in).  for $\ell\in S_{\mathrm{bad}}$ the local comparison is
made with respect to the N\'eron-model integral structure (so the component-group/Tamagawa correction is absorbed into
$\Delta_{\mathrm{BK},p}(E)$). Hence the transported scalar contributes no $\ell$-adic valuation to $u(E)$ for every $\ell\neq p_0$.
\cite{BlochKato1990,Nekovar2006,BLR1990NeronModels,SilvermanAECI}
\end{proof}
...
\begin{lemma}[Instantiation of \textsf{SME-R1} (Gross--Zagier calibration~\cite{GrossZagier1986})]
...
analytic regime. Then the Gross--Zagier formula~\cite{GrossZagier1986}, in the fixed period/height conventions of the pipeline, furnishes the
...
$\Delta_{\mathrm{BK},p}^{\mathrm{int}}(E)$. Moreover, the Gross--Zagier token~\cite{GrossZagier1986}
...
Then the Gross--Zagier calibration token~\cite{GrossZagier1986} $\mathbf{H}_{\mathrm{GZ}}(E,K;\mathcal{P}_K)$ fixes the \emph{global}
...
A \emph{transport isomorphism} is a $\Q$-linear isomorphism
\[
\Phi_{\mathrm{BK},\Q}(E):\ \Delta_{\mathrm{spec},\Q}(E)\xrightarrow{\ \sim\ }\Delta_{\mathrm{BK},\Q}(E)
\]
...
For each prime $p$ we write its base change as
\[
\Phi_{\mathrm{BK},p}(E)\ :=\ \Phi_{\mathrm{BK},\Q}(E)\otimes_{\Q}\Q_p:\ 
\Delta_{\mathrm{spec},p}(E)\xrightarrow{\ \sim\ }\Delta_{\mathrm{BK},p}(E).
\]
\end{definition}
...
\begin{proof}
By the Gate~L contract, for every unramified prime $\ell\notin S_{\mathrm{bad}}\cup\{p_0\}$ the local Selmer/determinant-line factor is normalized integrally, hence contributes no $\ell$-adic valuation to the comparison scalar (see Appendix~\ref{app:unramified} and the Selmer determinant-line formalism \cite{BlochKato1990,Nekovar2006}).
For each bad prime $\ell\in S_{\mathrm{bad}}\setminus\{p_0\}$, the ramified local block is absorbed into the canonical integral structure via the Tamagawa matching of Proposition~\ref{prop:LAI-tamagawa}, so again $v_\ell(u(E))=0$.
Combining these cases yields $v_\ell(u(E))=0$ for all $\ell\neq p_0$.
\end{proof}
...
Let $\Phi_{\mathrm{BK},\Q}(E):\Delta_{\mathrm{spec},\Q}(E)\to \Delta_{\mathrm{BK},\Q}(E)$ be the $\Q$-linear transport on determinant lines
provided by \DLT, and let $\Phi_{\mathrm{BK},p}(E)$ denote its base change to $\Q_p$.
Fix any nonzero reference elements $d_{\mathrm{spec},\Q}(E)\in\Delta_{\mathrm{spec},\Q}(E)$ and
$t_{\mathrm{BK},\Q}(E)\in\Delta_{\mathrm{BK},\Q}(E)$ coming from Gate~A2 and the Bloch--Kato/fundamental-line normalization.
Then there exists a unique scalar $u(E)\in\Q^\times$ such that
\[
\Phi_{\mathrm{BK},\Q}(E)\bigl(d_{\mathrm{spec},\Q}(E)\bigr)=u(E)\,t_{\mathrm{BK},\Q}(E).
\]
This rational scalar $u(E)$ is independent of admissible choices in the spectral realization and of auxiliary normalizations absorbed by Gate~L.
In contrast, the purely $p$-adic scalar $u_p(E)$ may change by a factor in $\Z_p^\times$ under $p$-adic basis/lattice changes, but its
\emph{global} component is fixed as $u_p^{\mathrm{glob}}(E)=\iota_p(u(E))$ (cf.\ \Cref{lem:DLT-global-local}).
\end{proposition}

\begin{proof}
Since $\Delta_{\mathrm{spec},\Q}(E)$ and $\Delta_{\mathrm{BK},\Q}(E)$ are one-dimensional $\Q$-lines, any $\Q$-linear map between them is multiplication
by a unique scalar once reference elements are fixed; this gives existence and uniqueness of $u(E)$.
Admissible changes on the spectral side (within the Gate~A2 realization) and on the arithmetic side (within the fixed Bloch--Kato/fundamental-line normalization,
including Gate~L's local conventions) rescale the chosen reference elements by nonzero \emph{rational} factors, hence do not alter the induced $u(E)\in\Q^\times$.
The remaining dependence on $p$-adic bases/lattices is absorbed into $\Z_p^\times$-ambiguity of $u_p(E)$ and does not affect $u(E)$.
(These are standard determinant-functor and Selmer-complex functoriality facts; see \cite{KnudsenMumford1976,Deligne1987,Nekovar2006,FukayaKato2006ETNC}.)
\end{proof}
...
\begin{lemma}[Rationality bridge]
\label{lem:URC-rationality-bridge}
The determinant-line transport package \DLT~\cite{KnudsenMumford1976,Deligne1987} provides a $\Q$-linear isomorphism of fundamental lines
\[
\Phi_{\mathrm{BK},\Q}(E):\Delta_{\mathrm{spec},\Q}(E)\longrightarrow \Delta_{\mathrm{BK},\Q}(E),
\]
whose base change to $\Q_p$ agrees with the $p$-adic transport $\Phi_{\mathrm{BK},p}(E)$.
Consequently there exists a unique scalar $u(E)\in\Q^\times$ such that, for any fixed nonzero reference elements
$d_{\mathrm{spec},\Q}(E)\in\Delta_{\mathrm{spec},\Q}(E)$ and $t_{\mathrm{BK},\Q}(E)\in\Delta_{\mathrm{BK},\Q}(E)$,
\[
\Phi_{\mathrm{BK},\Q}(E)\bigl(d_{\mathrm{spec},\Q}(E)\bigr)=u(E)\,t_{\mathrm{BK},\Q}(E).
\]
Moreover, for every place $v$ the localized scaling factor is the image $u_v(E)=\iota_v(u(E))\in\Q_v^\times$; in particular
$u_p^{\mathrm{glob}}(E)=\iota_p(u(E))\in\Q_p^\times$ and the working $p$-adic scalar differs only by a unit
$u_p(E)/u_p^{\mathrm{glob}}(E)\in\Z_p^\times$ (\Cref{lem:DLT-global-local}).
\end{lemma}
...
\begin{proof}
By \Cref{lem:URC-rationality-bridge}, $u(E)\in\Q^\times$. Since $v_\ell(u(E))=0$ for every $\ell\neq p_0$, the prime factorization of $u(E)$ may involve
...
\Cref{cor:afu-1g-from-lai}, this forces $\mathrm{Sel}_{p^\infty}(E/\Q)$ to be finite~\cite{Nekovar2006}, hence
...
to the comparison scalar (in the sense of \Cref{lem:LAI-ell-integrality}). Indeed, here the Bloch--Kato local condition is the unramified subspace and the induced determinant-line lattice is the canonical unramified $\Z_\ell$-lattice, so the local comparison factor lies in $\Z_\ell^{\times}$. \cite{BlochKato1990,Nekovar2006}
...
\nolinkurl{M0_IMC_SUPERSINGULAR_PLUSMINUS} & AFU-2:A0\_m, Index-ID, supersingular & r\_an=0; p: p odd, supersingular at p (a\_p = 0 or a\_p != 0 depending on cited package); red: good supersingular at p & supersingular at p (specify whether a\_p=0 is required); appropriate signed Selmer groups (±/signed) are used; main conjecture / divisibility in the signed setting applies (as proven in cited works); and other package-specific technical hypotheses as in the cited sources. & I: \Cref{thm:afu-upgrade}
\newline U: \Cref{prop:AFU2-indexID-conditional}\\
...
\newline U: \Cref{cor:afu-1g-from-lai}\\
\nolinkurl{M0_IMC_SUPERSINGULAR_AP0} & AFU-2:A0\_m, Index-ID, supersingular, a\_p=0 & r\_an=0; p: p odd, supersingular at p with a\_p=0 (covers the classical ± theory; p$\geq$3, often p$\geq$5 in some corollaries); red: good supersingular at p & supersingular at p with a\_p=0; signed (±) Selmer groups are used; signed IMC / reciprocity law in the a\_p=0 setting applies (as proven in the chosen reference); and other package-specific technical hypotheses as in the cited sources. & I: \Cref{thm:afu-upgrade}
\newline U: \Cref{prop:AFU2-indexID-conditional}\\
\nolinkurl{M0_IMC_SUPERSINGULAR_APNE0} & AFU-2:A0\_m, Index-ID, supersingular, a\_p!=0 & r\_an=0; p: p odd, supersingular at p with a\_p$\neq$0 (signed Selmer / sharp-flat or multi-signed variants); red: good supersingular at p & supersingular at p with a\_p != 0; appropriate signed Selmer groups are defined and used; signed IMC / reciprocity in the a\_p!=0 setting applies (as proven in the chosen reference); and other package-specific technical hypotheses as in the cited sources. & I: \Cref{thm:afu-upgrade}
\newline U: \Cref{prop:AFU2-indexID-conditional}\\
\nolinkurl{M1_RANK1_PPART_BSD} & AFU-2:M1 (rank-1 p-part Index-ID), Index-ID, rank-1 & r\_an=1; p: p odd; ordinary or supersingular depending on the chosen package; red: package-dependent (good at p typically) & analytic rank 1 (L'(E,1) != 0) and package-specific nonvanishing hypotheses; rank-1 arithmetic input available (Heegner/Kolyvagin or Kato-style rank-1 extension, depending on package); IMC / reciprocity in rank-1 setting applies (as proven in chosen references); and other package-specific technical hypotheses as in the cited sources. & I: \Cref{thm:afu-upgrade}
\newline U: \Cref{prop:AFU2-indexID-conditional}\\
\hline
\end{tabularx}
\normalsize
